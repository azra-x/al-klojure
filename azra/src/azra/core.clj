...
(defn curious [x] (if 
                    (= x (#(apply + (map faktorial (map read-string (map str  (seq (str  %)))))) x))
                    true
                    false))


;nanti euler-29 di map ke range dari 1 101

(def euler-291 "")

(def euler-29 (map read-string (map str euler-291)))

(defn euler [lst]
  (loop [lst lst res []]
  (if (<= (count lst) 12)
    res 
    (recur (rest lst) (conj res (take 13 lst))))))

(defn lala [x]
  (if (= x (apply + (map (comp (fn [x] (pangkat x 4)) read-string) (map str  (str x)))))
    true
    false))

(def euler30 (sum (filter lala (range 1000 1000000))))
